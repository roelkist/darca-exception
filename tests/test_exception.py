...
def test_darca_exception_basic():
    ...
    """
    exc = DarcaException(
        "Test error", error_code="TEST_001", metadata={"key": "value"}
    )
...
def test_darca_exception_default_values():
    """
    Test DarcaException initializes correctly when optional
...
def test_darca_exception_with_cause():
    ...
    exc = DarcaException(
        "Higher-level error", error_code="WRAP_001", cause=original_exception
    )
...
def test_darca_exception_to_dict():
    ...
    """
    exc = DarcaException(
        "Dict test", error_code="DICT_001", metadata={"key": "value"}
    )
...
def test_darca_exception_to_dict_with_cause():
    ...
    try:
        ...
        exc = DarcaException(
            "Wrapper error", error_code="WRAP_001", cause=cause
        )
        exc_dict = exc.to_dict()
...
def test_darca_exception_string_representation():
    ...
    # Strip spaces or handle any formatting inconsistencies
    assert (
        repr(exc).strip()
        == "DarcaException(error_code=STR_001, message=String test)"
    )
...
def test_darca_exception_string_representation_with_cause():
    ...
    try:
        ...
        exc = DarcaException(
            "Wrapper exception", error_code="WRAP_002", cause=cause
        )
