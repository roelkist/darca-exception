...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-exception-venv
    export POETRY_HOME := /tmp/poetry-exception-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-exception-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/exception-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-exception-cache
endif
